...
public class EmojiVacation {
    ...
    private static void generateVacationPhoto(CanvasWindow canvas) {
        ...
        if (percentChance(50)){
            addMountains(canvas, SCENE_HEIGHT, randomDouble(SCENE_WIDTH,SCENE_HEIGHT), randomInt(1, 3));
        }
    ...
    private static void positionFamily(
        ...
        double x = leftX;
        for (GraphicsGroup person : family) {
            double width = person.getWidth();
            double height = person.getHeight();
            person.setPosition(x, baselineY - height);
